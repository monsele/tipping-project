...
contract MintMeACoffeeFactory is Ownable {
     ...
     */
    function createCoffeeContract(string memory _name, string memory _symbol, address _creator)
        external
        returns (address)
    {
        // Allow either the platform or the creator themselves to create a contract
        require(msg.sender == owner() || msg.sender == _creator, "Not authorized to create contract");
        ...
        ICoffeeNFT(newContract).initialize(_name, _symbol, _creator, platformFeeReceiver);
