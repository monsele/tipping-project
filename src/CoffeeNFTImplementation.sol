...
contract CoffeeNFTImplementation is ERC1155, Ownable {
    ...
    function initialize(string memory _name, string memory _symbol, address _creator, address _platformFeeReceiver)
        external
    {
    ...
    function addCoffeeDesign(string memory _uri, uint256 _price, address _artistAddress, uint256 _artistRoyalty)
        external
        onlyOwner
        returns (uint256)
    {
...
function mintCoffee(uint256 tokenId, uint256 amount) external {
    ...
    IERC20(usdcToken).transferFrom(msg.sender, address(this), totalPayment);
    ...
    // Transfer the USDC
    require(IERC20(usdcToken).transfer(platformFeeReceiver, platformFee), "Platform fee transfer failed");
    
    if (artistRoyalty > 0) {
        require(IERC20(usdcToken).transfer(artistAddresses[tokenId], artistRoyalty), "Artist royalty transfer failed");
    }
    
    require(IERC20(usdcToken).transfer(creator, creatorPayment), "Creator payment failed");
